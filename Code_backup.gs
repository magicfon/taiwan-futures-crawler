...
function onOpen() {
  ...
  ui.createMenu('期貨資料')
    ...
    .addSubMenu(ui.createMenu('👥 身分別資料爬取')
      ...
      .addSeparator()
      .addItem('🧪 測試身分別數據抓取', 'testIdentityDataFetch')
    )
    .addSeparator()
        ...
        if (response && !hasNoDataMessage(response) && !hasErrorMessage(response)) {
            ...
            if (data) {
              ...
              if (isValid) {
                ...
                Logger.log(`⚠️ 增強版解析器獲取到數據但驗證未通過: ${data ? `[${data.slice(3, 8).join(', ')}]` : 'null'}`);
              }
            } else {
              Logger.log(`❌ 增強版解析器無法解析 ${contract} ${formattedDate} ${identity} 資料`);
            }
            ...
            if (data) {
              ...
              const isValid = validateIdentityData(data, identity) || 
                             ...
                             (data[3] > 0 || data[5] > 0); // 至少有一些交易數據
              
              if (isValid) {
                Logger.log(`✅ 成功使用標準解析器獲取 ${contract} ${formattedDate} ${identity} 資料`);
                Logger.log(`📊 數據摘要: 多方=${data[3]}, 空方=${data[5]}, 淨額=${data[7]}`);
              addDataToSheet(sheet, data);
              ...
              identitySuccess = true;
                ...
                Logger.log(`⚠️ 標準解析器獲取到數據但驗證未通過: ${data ? `[${data.slice(3, 8).join(', ')}]` : 'null'}`);
              }
            } else {
              Logger.log(`❌ 標準解析器無法解析 ${contract} ${formattedDate} ${identity} 資料`);
            }
            
            // 策略C: 嘗試絕對位置解析（特別針對NQF）
            if (contract === 'NQF') {
              const rowNumbers = {
                '自營商': [60, 61, 62, 59, 63], // 多個可能的行號
                '投信': [61, 62, 63, 60, 64],
                '外資': [62, 63, 64, 61, 65]
              };
...
function parseIdentityData(htmlContent, contract, identity, dateStr) {
  try {
    ...
    // 第一步：先找到對應契約的自營商行（作為基準）- 完全按照Python邏輯
    let baseRowIndex = -1;
    let highestScore = 0;
    ...
    for (let i = 0; i < rowMatches.length; i++) {
      ...
      if (rowText.includes('自營商') && (
          ...
          (CONTRACT_NAMES[contract] && rowText.includes(CONTRACT_NAMES[contract])) ||
          (contractPatterns[contract] && contractPatterns[contract].some(keyword => rowText.includes(keyword)))
      )) {
        currentScore += 20; // 這是找自營商基準行的關鍵
      }
      ...
      Logger.log(`${contract} 基準行搜尋 ${dateStr}: 第${i}行評分=${currentScore}, 內容="${rowText.substring(0, 80)}..."`);
      
      if (currentScore > highestScore && currentScore >= 25) { // 提高最低分數門檻，確保找到正確的自營商行
    ...
    Logger.log(`${contract} ${identity} ${dateStr}: 找到 ${contract} 自營商基準行在第 ${baseRowIndex} 行，得分=${highestScore}`);
    
    // 第二步：根據身份別確定目標行索引（完全按照Python邏輯）
    let targetRowIndex = -1;
    
    if (identity === '自營商') {
      targetRowIndex = baseRowIndex;
    } else if (identity === '投信') {
      targetRowIndex = baseRowIndex + 1; // 投信在自營商的下一行
    } else if (identity === '外資') {
      targetRowIndex = baseRowIndex + 2; // 外資在自營商的下兩行
    } else {
    ...
    if (targetRowIndex < 0 || targetRowIndex >= rowMatches.length) {
      Logger.log(`${contract} ${identity} ${dateStr}: 目標行索引超出範圍: ${targetRowIndex}/${rowMatches.length}`);
      return null;
    }
    
    const targetRow = rowMatches[targetRowIndex];
    Logger.log(`${contract} ${identity} ${dateStr}: 使用絕對位置第${targetRowIndex}行`);
    
    // 第三步：解析目標行資料
    const cellMatches = targetRow.match(/<td[^>]*>[\s\S]*?<\/td>/gi);
    if (!cellMatches || cellMatches.length < 5) {
      Logger.log(`${contract} ${identity} ${dateStr}: 目標行的單元格數量不足，實際=${cellMatches ? cellMatches.length : 0}`);
      return null;
    }
    ...
    Logger.log(`${contract} ${identity} ${dateStr}: 目標行包含${cellContents.length}個單元格`);
    Logger.log(`${contract} ${identity} ${dateStr}: 單元格內容=[${cellContents.slice(0, 8).join(' | ')}...]`);
    
    // 檢查第一欄是否包含身份別
    const isIdentityFirst = cellContents.length > 0 && 
      (cellContents[0] === identity || cellContents[0].includes(identity));
    
    Logger.log(`${contract} ${identity} ${dateStr}: 身份別在第一欄=${isIdentityFirst}`);
    
    // 根據表格格式確定數據映射 - 參考Python版本的mapping邏輯
    let indices = {};
    
    if (isIdentityFirst) {
      // 身份別在第一欄的格式 - 參考Python版本
      indices = {
        '多方交易口數': 1,
        '多方契約金額': 2,
        '空方交易口數': 3,
    ...
    Logger.log(`${contract} ${identity} ${dateStr}: 絕對位置解析結果=[${dataArray.slice(3, 8).join(' | ')}...]`);
    
    // 基本驗證：檢查是否有有效數據
    const hasValidData = dataArray.slice(3).some(value => value !== 0);
    if (!hasValidData) {
      Logger.log(`${contract} ${identity} ${dateStr}: 警告：所有數值欄位都為0`);
      // 不直接返回null，讓上層決定是否接受全零資料
    }
...
// 專門測試NQF資料抓取的函數
function testNQFDataFetchNoUI() {
  try {
    // 硬編碼測試日期 - 請修改為您想測試的日期
    const testDate = '2024/12/20'; // 修改這裡為您想測試的日期
    
    Logger.log(`=== 開始NQF專門測試 (無UI版本) ===`);
    Logger.log(`測試日期: ${testDate}`);
    ...
    for (const identity of identities) {
      ...
      for (const queryType of queryTypes) {
        for (const marketCode of marketCodes) {
          Logger.log(`\n--- 測試參數組合: queryType=${queryType}, marketCode=${marketCode} ---`);
          ...
          try {
            ...
            if (response) {
              // 檢查回應內容
              const hasNoData = hasNoDataMessage(response);
              const hasError = hasErrorMessage(response);
              const hasNQF = response.includes('NQF') || response.includes('那斯達克') || response.includes('Nasdaq');
              
              Logger.log(`回應狀態: 無資料=${hasNoData}, 錯誤=${hasError}, 包含NQF=${hasNQF}`);
              
              if (!hasNoData && !hasError && hasNQF) {
                // 測試增強版解析器
                const enhancedData = enhancedParseIdentityData(response, testDate, identity);
                
                if (enhancedData) {
                  Logger.log(`✅ 增強版解析器成功！`);
                  Logger.log(`📊 ${identity}: 多方=${enhancedData[3]}, 空方=${enhancedData[5]}, 淨額=${enhancedData[7]}`);
                  Logger.log(`📊 未平倉: 多方=${enhancedData[9]}, 空方=${enhancedData[11]}, 淨額=${enhancedData[13]}`);
                  ...
                  // 測試標準解析器
                  const standardData = parseIdentityData(response, contract, testDate, identity);
                  
                  if (standardData && validateIdentityData(standardData, identity)) {
                    Logger.log(`✅ 標準解析器成功！`);
                    Logger.log(`📊 ${identity}: 多方=${standardData[3]}, 空方=${standardData[5]}, 淨額=${standardData[7]}`);
                    ...
                    try {
                      const ss = SpreadsheetApp.getActiveSpreadsheet();
                      let debugSheet = ss.getSheetByName('NQF調試');
                      if (!debugSheet) {
                        debugSheet = ss.insertSheet('NQF調試');
                        // 添加表頭
                        debugSheet.getRange('A1:G1').setValues([
                          ['時間', '測試日期', '身份別', 'QueryType', 'MarketCode', '狀態', '回應內容(前500字)']
                        ]);
                        debugSheet.getRange('A1:G1').setFontWeight('bold');
                      }
                      ...
                      Logger.log(`無法寫入調試工作表: ${sheetError.message}`);
                    }
                  }
                }
              } else {
                Logger.log(`❌ 回應品質不佳: 無資料=${hasNoData}, 錯誤=${hasError}, 包含NQF=${hasNQF}`);
                
                // 如果沒有NQF相關內容，檢查回應的前200個字符
                if (!hasNQF) {
                  const preview = response.substring(0, 200).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
                  Logger.log(`回應預覽: ${preview}`);
                }
              }
            } else {
              Logger.log(`❌ 無法獲取回應`);
    ...
    // 生成測試報告
    Logger.log(`\n=== NQF測試報告 ===`);
    let reportText = `NQF資料抓取測試報告\n測試日期: ${testDate}\n\n`;
    
    const successResults = allResults.filter(r => r.success);
    
    if (successResults.length > 0) {
      reportText += `✅ 成功的參數組合 (${successResults.length}個):\n`;
      
      for (const result of successResults) {
        reportText += `${result.identity}: queryType=${result.queryType}, marketCode=${result.marketCode} (${result.parser}解析器)\n`;
        reportText += `  多方: ${result.data[3]}, 空方: ${result.data[5]}, 淨額: ${result.data[7]}\n`;
        reportText += `  未平倉多方: ${result.data[9]}, 未平倉空方: ${result.data[11]}\n\n`;
      }
    } else {
      reportText += `❌ 沒有成功的參數組合\n`;
      reportText += `建議檢查：\n`;
      reportText += `1. 日期是否為交易日 (${testDate})\n`;
      reportText += `2. NQF在該日是否有交易\n`;
      reportText += `3. 期交所網站是否正常\n`;
      reportText += `4. 契約代碼是否正確\n\n`;
    }
    
    // 檢查是否有重複數據的問題
    const identityGroups = {};
    for (const result of successResults) {
      if (!identityGroups[result.identity]) {
        identityGroups[result.identity] = [];
      }
      identityGroups[result.identity].push(result);
    }
    
    let hasDuplicates = false;
    for (const [identity, results] of Object.entries(identityGroups)) {
      if (results.length > 1) {
        hasDuplicates = true;
        reportText += `⚠️ ${identity} 有多個成功的參數組合，數據可能不一致:\n`;
        
        for (const result of results) {
          reportText += `  queryType=${result.queryType}, marketCode=${result.marketCode}: 多方=${result.data[3]}, 空方=${result.data[5]}\n`;
        }
        reportText += '\n';
        ...
        if (results.length > 1) {
          ...
          for (let i = 0; i < results.length; i++) {
            const r = results[i];
            Logger.log(`  方案${i+1}: queryType=${r.queryType}, marketCode=${r.marketCode}`);
            Logger.log(`    多方交易: ${r.data[3]}, 空方交易: ${r.data[5]}, 交易淨額: ${r.data[7]}`);
            Logger.log(`    多方未平倉: ${r.data[9]}, 空方未平倉: ${r.data[11]}, 未平倉淨額: ${r.data[13]}`);
          }
        }
      }
    }
    ...
    if (hasDuplicates) {
      reportText += `🔍 發現重複數據問題！不同參數組合返回不同結果，這可能是造成錯誤數據的原因。\n`;
      reportText += `建議選擇最可信的參數組合或進一步驗證數據來源。\n\n`;
    }
    ...
    if (successResults.length > 0) {
      ...
      // 為每個身份別推薦最佳參數組合
      for (const identity of identities) {
        const identityResults = successResults.filter(r => r.identity === identity);
        if (identityResults.length > 0) {
          // 優先選擇增強版解析器的結果
          const bestResult = identityResults.find(r => r.parser === '增強版') || identityResults[0];
          Logger.log(`  ${identity}: queryType=${bestResult.queryType}, marketCode=${bestResult.marketCode} (${bestResult.parser}解析器)`);
        } else {
          Logger.log(`  ${identity}: 無可用參數組合`);
        }
      }
    }
    ...
    return {
      ...
      totalTests: queryTypes.length * marketCodes.length * identities.length,
    ...
    Logger.log(`NQF測試時發生錯誤: ${e.message}`);
    Logger.log(`錯誤堆疊: ${e.stack}`);
    throw e;
  }
}
...
// 快速測試特定參數組合的NQF資料
function quickTestNQF(testDate = '2024/12/20', queryType = '2', marketCode = '1', identity = '外資') {
  try {
    Logger.log(`=== 快速NQF測試 ===`);
    Logger.log(`參數: 日期=${testDate}, queryType=${queryType}, marketCode=${marketCode}, 身份別=${identity}`);
    ...
    if (response) {
      ...
      Logger.log(`回應狀態: 長度=${response.length}, 無資料=${hasNoData}, 錯誤=${hasError}, 包含NQF=${hasNQF}`);
      ...
      if (!hasNoData && !hasError && hasNQF) {
        ...
        if (enhancedData) {
          Logger.log(`✅ 增強版解析器成功解析 ${identity} 資料:`);
          Logger.log(`📊 多方交易: ${enhancedData[3]}, 空方交易: ${enhancedData[5]}, 交易淨額: ${enhancedData[7]}`);
          Logger.log(`📊 多方未平倉: ${enhancedData[9]}, 空方未平倉: ${enhancedData[11]}, 未平倉淨額: ${enhancedData[13]}`);
          return enhancedData;
        } else {
          Logger.log(`❌ 增強版解析器失敗，嘗試標準解析器`);
          
          const standardData = parseIdentityData(response, 'NQF', testDate, identity);
          if (standardData) {
            Logger.log(`✅ 標準解析器成功解析 ${identity} 資料:`);
            Logger.log(`📊 多方交易: ${standardData[3]}, 空方交易: ${standardData[5]}, 交易淨額: ${standardData[7]}`);
            return standardData;
          } else {
            Logger.log(`❌ 標準解析器也失敗`);
            
            // 輸出一些回應內容供分析
            const preview = response.substring(0, 1000).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
            Logger.log(`回應預覽: ${preview}`);
          }
        }
      } else {
